<template>
  ...
  <div class="main-boxdiv" v-loading="Loadingodds">  
    <div class="mb-4 total_nav_span">
      <span>总收：<span>{{acq ? parseFloat(All_total.Zcje.toFixed(2)):parseFloat(All_total.BetMoney.toFixed(2))}}</span></span>
      <span>佣金：<span class="Rate">{{acq ? parseFloat(All_total.Zcty.toFixed(2)):parseFloat(All_total.Ty.toFixed(2))}}</span></span>
      <span>总实收：<span :class="[acq ? (All_total.Zcje+All_total.Zcty)<0 ? 'Rate':'':'']">{{acq ? parseFloat((All_total.Zcje+All_total.Zcty).toFixed(2)):parseFloat((All_total.BetMoney+All_total.Ty).toFixed(2))}}</span></span>
    </div>     
   <div class="game_herad"> 
     <div class="game_herad_tab" style="margin:0 auto">
       <div class="game_herad_tab_nav" v-for="(items,index) in Play_Grpup_data" :class="index === default_Group ? 'isActive1' : ''" @click="QuickChooseDatas=[],Play_Group_Check(index,0,items)">
         <div class="game_herad_tab_nav_a" :class="animation[index]" :style="{'align-items':index==0 ? 'center':''}">
   ...
   <div class="mb-4">
    <span v-for="item in gettotal_num()"><span class="total_name">{{item.name}}</span>:<span class="total_nav_span">{{parseFloat((acq ? item.Zcje:item.BetMoney).toFixed(2))}}</span>/<span :class="[acq ? item.Zcty<0 ? 'Rate':'':item.Ty<0 ? 'Rate':'']" class="total_nav_span">{{parseFloat((acq ? item.Zcty:item.Ty).toFixed(2))}}</span>/<span :class="[acq ? (item.Zcje+item.Zcty)<0 ? 'Rate':'':(item.BetMoney+item.Ty)<0 ? 'Rate':'']" class="total_nav_span">{{acq ? parseFloat((item.Zcje+item.Zcty).toFixed(2)):parseFloat((item.BetMoney+item.Ty).toFixed(2))}}</span> </span>
   </div>
   <div class="psc_odds_box">
     <div class="psc_odds_box_top titlem">  
      <div v-if="Play_Grpup_data[default_Group].play_type.length>1 && Play_Grpup_data[default_Group].alignModel!=3">
        <span>项目</span>
        <span class="psc_odds_box_topbtn" v-for="(items,index) in Play_Grpup_data[default_Group].play_type" :class="default_PlayType==index ? 'isActive2':''" @click="playtypechan(index)">{{items.name}}</span>
      </div>   
      <div v-if="Play_Grpup_data[default_Group].PlayZt">
        <span>玩法</span>
        <span class="psc_odds_box_topbtn" v-for="items in Play_Grpup_data[default_Group].PlayZt" :class="default_data.PlayZt==items ? 'isActive1':''" @click="Play_Group_Check(default_Group,default_PlayType,Play_Grpup_data[default_Group],items)">正{{items}}</span>
      </div>       
      <div>
        <span>类型</span>
        <span>
   ...
   <div :class="[Play_Grpup_data[default_Group].alignModel==3 ? 'monitor_table_align':'',Play_Grpup_data[default_Group].alignModel==2 ? 'monitor_alignModel2_box':'']">
      <el-table  v-for="(items,indexs) in  getTableData()" border class="monitor_table" :span-method="objectSpanMethod" :header-cell-class-name="'trheader'" :data="items.tableData"  :height="Play_Grpup_data[default_Group].alignModel!=2 ? clientHeight1+'px':'auto'" :style="GetTableStyle()" :cell-class-name="cellClassName">
        ...
        <el-table-column type="index" v-if="Play_Grpup_data[default_Group].alignModel!=3 && Play_Grpup_data[default_Group].alignModel!=2" label="序号" />
        <el-table-column :label="Play_Grpup_data[default_Group].alignModel==3 ||  Play_Grpup_data[default_Group].alignModel==2 ? items.name:'球号'" :width="Play_Grpup_data[default_Group].alignModel>=3 ? '':'100'">
          <template  #default="{ row,$index }">
            ...
            <span @click="GetOrderListPost=row,dskey=dskey+1,OrderShow=true" v-if="Play_Grpup_data[default_Group].alignModel!=3" class="play_text ball_n" :class="GetnumberClass(row)" :style="GetnumberStyle(row)">  
              <span class="text_nowrap" :title="row.PlayNumber">{{ row.PlayNumber }}</span>                         
            </span> 
            <div class="monitor_table_cell_div" style="justify-content:space-between;" v-if="Play_Grpup_data[default_Group].alignModel==3" >
              <div class="text_nowrap" :title="row.PlayNumber" @click="GetOrderListPost=row,dskey=dskey+1,OrderShow=true">{{ row.PlayNumber }}={{ (acq ? row.ZcBetMoney:row.BetMoney).toFixed(1) }}</div>
              <div class="text_nowrap">
                <span class="color_red" @click="bh(1,items.tableData,$index,items.name)">单补</span>
                <span class="color_red" v-if="$index>0" @click="bh(2,items.tableData,$index,items.name)">多补</span>
              </div>                
            </div>              
        ...
        <el-table-column label="占成金额" v-if="default_Group!=0 && default_Group!=7">
          <template  #default="{ row,$index }">
            <div class="monitor_table_cell_div">
              ...
              <div class="text_nowrap color_red" v-if="$index>0" @click="bh(2,items.tableData,$index,items.name)">多补</div>            
            </div>            
          </template> 
        </el-table-column> 
        <el-table-column label="退佣" v-if="default_Group!=0 && default_Group!=7">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
            <div class="text_nowrap" :class="[acq ? row.Zcty!=0 ? 'Rate':'':row.Ty!=0 ? 'Rate':'']">{{ (acq ? row.Zcty:row.Ty).toFixed(2) }}</div> 
            </div>          
          </template> 
        </el-table-column>     
        ...
        <el-table-column v-if="default_Group==1 || default_Group==3 || default_Group==4 || default_Group==0" label="最大盈亏">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.Maxyk<0 ? 'Rate':'':row.XuMaxyk<0 ? 'Rate':'']">{{(acq ? row.Maxyk:row.XuMaxyk).toFixed(1)}}</div> 
            </div>                
          </template> 
        </el-table-column> 
        ...
        <el-table-column label="特码"  width="180" v-if="default_Group==0">
          <template  #default="{ row,$index }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyTm<0 ? 'Rate':'':row.BetMoneyTm<0 ? 'Rate':'']"  @click="GetOrderListPost=row,dskey=dskey+1,OrderShow=true">{{(acq ? row.ZcBetMoneyTm:row.BetMoneyTm).toFixed(1) }}</div> 
                <div class="text_nowrap color_red" @click="bh(1,items.tableData,$index,items.name)">单补</div>       
                <div class="text_nowrap color_red" v-if="$index>0" @click="bh(2,items.tableData,$index,items.name)">多补</div>                  
            </div>               
          </template> 
        ...
        <el-table-column label="两面" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyLm<0 ? 'Rate':'':row.BetMoneyLm<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyLm:row.BetMoneyLm).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>     
        <el-table-column label="波色" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyBs<0 ? 'Rate':'':row.BetMoneyBs<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyBs:row.BetMoneyBs).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>              
        <el-table-column label="特肖" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneySx<0 ? 'Rate':'':row.BetMoneySx<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneySx:row.BetMoneySx).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>    
        <el-table-column label="特头" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyTs<0 ? 'Rate':'':row.BetMoneyTs<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyTs:row.BetMoneyTs).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>      
        <el-table-column label="特尾" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyWs<0 ? 'Rate':'':row.BetMoneyWs<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyWs:row.BetMoneyWs).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>      
        <el-table-column label="半特" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyBt<0 ? 'Rate':'':row.BetMoneyBt<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyBt:row.BetMoneyBt).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>    
        <el-table-column label="半波" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyBb<0 ? 'Rate':'':row.BetMoneyBb<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyBb:row.BetMoneyBb).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>  
        <el-table-column label="半半波" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyBbb<0 ? 'Rate':'':row.BetMoneyBbb<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyBbb:row.BetMoneyBbb).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>     
        <el-table-column label="五行" v-if="default_Group==0">
          <template  #default="{ row }">
            <div class="monitor_table_cell_div">
              <div class="text_nowrap" :class="[acq ? row.ZcBetMoneyWx<0 ? 'Rate':'':row.BetMoneyWx<0 ? 'Rate':'']">{{(acq ? row.ZcBetMoneyWx:row.BetMoneyWx).toFixed(1) }}</div> 
            </div>               
          </template> 
        </el-table-column>                                  
      </el-table>  
   ...
   <el-dialog v-model="bhdia" title=""  :class="bhtype==2 ? 'Bh-dialogs':'Bh-dialog'">
      ...
      <div>
        ...
        <span v-if="default_Group<4">
          <select v-model="BhAb" @change="changeBhAb()">
            <option v-for="(item,index) in BhAbData[default_Group].data" :label="item.name" :value="item.playtype"/>
          </select>      
        </span>         
      </div>
      <div>
     ...
     <el-table  border class="monitor_table monitor_tabledia" height="100%" style="width: 100%" :header-cell-class-name="'trheader'" :data="ShowListData" v-loading="LoadingBh"> 
      ...
      <el-table-column label="赔率">
        <template  #default="{ row,$index }">
                ...
                <span v-if="row.Rate2>0" class="Rate">/<input type="text" v-model="row.Rate2"  class="monitor_table_bh_input" /></span>
        </template> 
      </el-table-column>   
      ...
      <el-table-column label="补货金额">
        <template  #default="{ row,$index }">
            <input type="text" v-model="row.BetMoney"  class="monitor_table_bh_input" @click="changeBhBetMoney($index)"/>
        </template> 
      </el-table-column>                        
     </el-table> 
   ...
   <el-dialog v-model="OrderShow" title="" class="dssetCom-dialog" width="850">
    <div class="titlem bodynav dialog-padding">
     ...
     <component :is="'detail'" :key="dskey" :GameList="GameList" :GetOrderListPost="GetOrderListPost" :PlayZt="default_data.PlayZt" :CompanyId="default_data.CompanyId"></component>         
  </el-dialog>  
