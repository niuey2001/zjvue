   ...
   <div class="psc_odds_box" v-loading="Loadingodds">
     <div class="psc_odds_box_top titlem">       
       ...
       <div v-if="Play_Grpup_data[default_Group].PlayZt">
         <span class="psc_odds_box_topbtn" v-for="items in Play_Grpup_data[default_Group].PlayZt" :class="default_data.PlayZt==items ? 'isActive1':''" @click="Play_Group_Check(default_Group,default_PlayType,Play_Grpup_data[default_Group],items)">{{Play_Grpup_data[default_Group].PlayZt_txt}}{{items}}</span>
       </div> 
       <div>全盘操作</div>    
       <div>
         <span class="psc_odds_box_topbtn" v-if="default_Group==0" :class="IsOpenGame.tm ? 'isActive1':''" @click="upopenstate(true,[1,2,3,4])">
           特码收单         
         </span>  
         <span class="psc_odds_box_topbtn" v-if="default_Group==0" :class="IsOpenGame.ftm ? 'isActive1':''" @click="upopenstate(true,[])">
           非特码收单         
         </span> 
         <span class="psc_odds_box_topbtn" v-if="default_Group==0" :class="!IsOpenGame.tm ? 'isActive1':''" @click="upopenstate(false,[1,2,3,4])">
           特码封单         
         </span>  
         <span class="psc_odds_box_topbtn" v-if="default_Group==0" :class="!IsOpenGame.ftm ? 'isActive1':''" @click="upopenstate(false,[])">
           非特码封单         
         </span>         
     ...
     <div class="psc_odds_box_top"> 
       ...
       <div>
         ...
         <span style="display: none;" class="psc_odds_box_topbtn" @click="OpenChangeBox(batchsetodds=true,Odds_data)">
           批量控赔         
         </span>    
       ...
       <div style="border-right:0px ;" v-if="Play_Grpup_data[default_Group].play_type.length>1 && default_Group!=0"> 
         <span>{{Play_Grpup_data[default_Group].group_name}}收封单</span>
         <span class="psc_odds_box_topbtn" :class="Play_Grpup_data[default_Group].openstate ? 'isActive2':''" @click="upopenstate(true,Play_Grpup_data[default_Group].opendata)">收单</span>
         <span class="psc_odds_box_topbtn" :class="!Play_Grpup_data[default_Group].openstate ? 'isActive2':''" @click="upopenstate(false,Play_Grpup_data[default_Group].opendata)">封单</span>         
       </div>                
       <div style="border-right:0px ;"> 
         <span>单盘收封单</span>
         <span class="psc_odds_box_topbtn" :class="Play_Grpup_data[default_Group].openstate ? 'isActive2':''" @click="upopenstate(true,default_data.PlayType)"><span v-if="Play_Grpup_data[default_Group].play_type.length>1">{{Play_Grpup_data[default_Group].play_type[default_PlayType].name}}</span>收单</span>
         <span class="psc_odds_box_topbtn" :class="!Play_Grpup_data[default_Group].openstate ? 'isActive2':''" @click="upopenstate(false,default_data.PlayType)"><span v-if="Play_Grpup_data[default_Group].play_type.length>1">{{Play_Grpup_data[default_Group].play_type[default_PlayType].name}}</span>封单</span>                
       </div>           
     </div>
     <div class="psc_odds_box_list">        
       <el-table  :header-cell-class-name="'trheader'" v-for="(items,key,index) in Play_Grpup_data[default_Group].tableNum" :key="key" border :data="items.tableDta" style="margin-left: -1px;" :style="{'margin-right':index==Object.keys(Play_Grpup_data[default_Group].tableNum).length-1 ? '-1px':''}">     
         <el-table-column :label="key">
           <template #header>
             <div  class="tableheader">
               <span>{{ key }}</span>  
               <el-popconfirm v-if="Play_Grpup_data[default_Group].openbtn"  :title="'确定将'+key+(items.openstate ? '封单':'收单')+'?'" confirm-button-type="danger"  @confirm="upopenstate(!items.openstate,items.playtype)" confirm-button-text="确认" cancel-button-text="取消">
                 <template #reference>
           ...
           <template  #default="{ row }">
               <div class="tableheader" :style="{'background':row.State ? '#e8e6e6':''}">
                 <div style="width: 50%;max-width:24%;overflow: hidden;text-align: left;">
                   <div class="play_text ball_n" :class="row.NumberBs==66 ? 'ball_red':row.NumberBs==67 ? 'ball_blue':row.NumberBs==68 ? 'ball_green':''" :style="{'color':row.NumberItem==66 || [90,91,92,93,102,103,104,105,177].includes(row.NumberItem) ? 'red':row.NumberItem==67 || [94,95,96,97,106,107,108,109,178].includes(row.NumberItem) ? 'blue':row.NumberItem==68 || [98,99,100,101,110,111,112,113,179].includes(row.NumberItem) ? 'green':''}">  
                   <span class="text_nowrap" :title="row.PlayNumber">{{ row.PlayNumber }}</span>             
                   </div> 
                 </div>                
                 <div class="setRate">
                     <span class="up" @click="setoddschange([row],0)">+</span>
                     <span @click="openSetOdds(row)"  :title="Play_Grpup_data[default_Group].play_type[default_PlayType].secondrate ? row.Rate1+'/'+row.Rate2:row.Rate1" class="Rate text_nowrap">{{ row.Rate1 }}<span v-if="Play_Grpup_data[default_Group].play_type[default_PlayType].secondrate">/{{ row.Rate2 }}</span></span>
                     <span class="up" @click="setoddschange([row],1)">-</span>                    
                 </div>
                 <div :class="default_Group==14 ? 'total_box1':'total_box'">
                   <span :title="acq ? row.ZcBetMoney:row.BetMoney">{{parseInt(acq ? row.ZcBetMoney:row.BetMoney)}}</span>
                   <span v-if="default_Group==0 || default_Group==2 || default_Group==3" :style="{'color':acq ? row.Maxyk<0 ? 'red':'':row.XuMaxyk<0 ? 'red':''}"  :title="acq ? row.Maxyk:row.XuMaxyk">{{parseInt(acq ? row.Maxyk:row.XuMaxyk)}}</span>
                 </div>                
               </div>                
           </template> 
         </el-table-column>                
          ...
          <table border="1" class="el-form-table">
               <tr>
                 ...
                 <td class="el-form-table-tr" style="font-weight:600;">
                   <div class="play_text ball_n" style="white-space: nowrap;" :class="oddssetdata.NumberBs==66 ? 'ball_red':oddssetdata.NumberBs==67 ? 'ball_blue':oddssetdata.NumberBs==68 ? 'ball_green':''" :style="{'color':oddssetdata.NumberItem==66 || [90,91,92,93,102,103,104,105,177].includes(oddssetdata.NumberItem) ? 'red':oddssetdata.NumberItem==67 || [94,95,96,97,106,107,108,109,178].includes(oddssetdata.NumberItem) ? 'blue':oddssetdata.NumberItem==68 || [98,99,100,101,110,111,112,113,179].includes(oddssetdata.NumberItem) ? 'green':''}">
