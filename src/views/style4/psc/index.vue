      ...
      <div class="psc_odds_box_top titlem">
        ...
        <div v-if="Play_Grpup_data[default_Group].PlayZt">
          <span class="psc_odds_box_topbtn" v-for="items in Play_Grpup_data[default_Group].PlayZt"
            ...
            @click="Play_Group_Check(default_Group, default_PlayType, Play_Grpup_data[default_Group], items)">{{ Play_Grpup_data[default_Group].PlayZt_txt }}{{ items }}</span>
        </div>
        <div>全盘操作</div>
        <div>
        ...
        <div style="border-right:0px ;">
          ...
          <span class="psc_odds_box_topbtn" :class="Play_Grpup_data[default_Group].openstate ? 'isActive2' : ''"
            @click="upopenstate(true, default_data.PlayType)"><span
              v-if="Play_Grpup_data[default_Group].play_type.length > 1">{{ Play_Grpup_data[default_Group].play_type[default_PlayType].name }}</span>收单</span>
          <span class="psc_odds_box_topbtn" :class="!Play_Grpup_data[default_Group].openstate ? 'isActive2' : ''"
            @click="upopenstate(false, default_data.PlayType)"><span
              v-if="Play_Grpup_data[default_Group].play_type.length > 1">{{ Play_Grpup_data[default_Group].play_type[default_PlayType].name }}</span>封单</span>
        </div>
      </div>
      <div class="psc_odds_box_list">
        <el-table :header-cell-class-name="'trheader'"
          v-for="(items, key, index) in Play_Grpup_data[default_Group].tableNum" :key="key" border :data="items.tableDta"
