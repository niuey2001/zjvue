<template>
  <div class="main-boxdiv" v-loading="loading">
    ...
    <div class="game_box_top game_box_top_nav" style="margin-right: 0px;border-left:1px solid #b9c2cb;border-right:1px solid #b9c2cb;line-height: 25px;background: rgb(248, 208, 150);">
      <div style="justify-content: center !important;
    align-items: center !important;
    line-height: 20px;
    display: flex;background: rgb(248, 208, 150);" >
        ...
        <span class="game_box_topbtn" @click="updatesave()">保存降赔设置</span> 
        <span class="game_box_topbtn" @click="resdata()">重置输入</span> 
      </div>      
    </div> 
   ...
   <div style="display:flex;justify-content: space-between;">
      ...
      style="width: 100%;" 
      show-footer
      v-for="(item,index) in editBhdata"
      :data="item" 
      :style="{'margin-left':index>0 ? '-1px':'0'}"
    >   
        ...
        <el-table-column label="自动降赔" width="100">
          <template  #default="{ row }">
            <span>
             <select v-model="row.State" :style="{'background-color':row.State ? '#fef5a2 !important':'#ffffff !important'}">
