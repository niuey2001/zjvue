    ...
    <div ref="maintopmenu" class="game_herad">
      <div class="game_herad_tab">
        ...
        <div class="game_herad_tab_nav" v-for="(items, index) in Play_Grpup_data"
          :class="index === Play_Group_Index ? 'isActive1' : ''" @click="changePlayGroup(index)">
    ...
    <el-table border :data="editData" :header-cell-class-name="'trheader'" :height="clientHeight2 + 'px'"
      ...
      <el-table-column label="A盘赔率" width="180">
        <template #default="{ row, $index }">
          <input :class="parseFloat(OldeditData[$index].odds1) != parseFloat(row.odds1) ? 'Odds_input_focus' : ''"
            class="Odds_input" type="number" v-model="row.odds1" @focus="handlefocus(row)" />
          <span v-if="row.is_two">/
            <input :class="parseFloat(OldeditData[$index].odds2) != parseFloat(row.odds2) ? 'Odds_input_focus' : ''"
              class="Odds_input" type="number" v-model="row.odds2" />
          </span>
      ...
      <el-table-column v-for="(items, key) in keyname" :label="items">
        <template #default="{ row, $index }">
          <input :class="parseFloat(OldeditData[$index][key]) != parseFloat(row[key]) ? 'Odds_input_focus' : ''"
            class="Odds_input" type="number" v-model="row[key]" />
        </template>
      </el-table-column>
      <el-table-column label="是否开放">
        <template #header>
          <span>状态</span><br />
          <el-switch v-model="allstate" @change="chstate()" />
        </template>
        <template #default="{ row }">
          <el-switch v-model="row.self_state" />
        </template>
      </el-table-column>
    </el-table>
