<template>
  <div class="main-boxdiv" v-loading="loading">
    ...
    <el-dialog v-model="dialogFormVisible" title=""
      :class="componentsPoPdata.opencomponents == 'dssetCom' || componentsPoPdata.opencomponents == 'logCom' ? 'dssetCom-dialog' : ''"
      width="850">
      <div class="titlem bodynav dialog-padding">
        {{ componentsPoPdata.title }}
      </div>
      <component :is="componentsPoPdata.opencomponents" :key="dskey" :home_data="home_data"
        @opencomponentsBox="opencomponentsBox" @getdata="getdata" :MemberType="MemberType"
        :componentsPoPdata="componentsPoPdata" :Versions="Versions"></component>
    </el-dialog>
    ...
    <el-table border :header-cell-class-name="'trheader'"
      ...
      <el-table-column
        :label="componentsPoPdata.nowdata.type < 3 ? MemberType[componentsPoPdata.nowmember.type] + '占成' : '上级占成'"
        width="105">
        <template #default="{ row }">
      ...
      <el-table-column label="信用额度">
        <template #default="{ row }">
          <input :ref="`name-${row.id}`" v-model="tabClickTs" v-show="row.id === tabClickId && tabClickLabel === '信用额度'"
            placeholder="信用额度" class="Odds_input" style="height:auto" />
      ...
      <el-table-column label="启/停用" width="70">
        <template #default="{ row }">
          <el-popconfirm :title="'确定将' + row.account + (row.state ? '停用' : '启用') + '?'" confirm-button-type="danger"
            @confirm="changeState(row)" confirm-button-text="确认" cancel-button-text="取消">
            <template #reference>
      ...
      <el-table-column label="冻结" width="60">
        <template #default="{ row }">
          <el-popconfirm :title="'确定将' + row.account + (row.is_bet ? '冻结' : '解除冻结') + '?'" confirm-button-type="danger"
            @confirm="changeBetState(row)" confirm-button-text="确认" cancel-button-text="取消">
            <template #reference>
    ...
    <div class="mb-4">
      <el-pagination background layout="total,prev, pager, next" v-model:current-page="componentsPoPdata.nowdata.page"
        v-model:page-size="componentsPoPdata.nowdata.size" :total="total" @size-change="handleSizeChange"
