<template>
  <div class="main-boxdiv1" style="width: 100%;height:92%;">
    <div class="bodynav">
      <el-button type="primary" :class="logdata.index == 1 ? 'is-active' : ''"
        @click="logdata.index = 1, logdata.page = 1, logdata.total = 0, logs(logdata.id, logdata.account)">操作记录</el-button>
      <el-button type="primary" :class="logdata.index == 2 ? 'is-active' : ''"
        @click="logdata.index = 2, logdata.page = 1, logdata.total = 0, logs(logdata.id, logdata.account)">登录日志</el-button>
    </div>
    <el-table border :header-cell-class-name="'trheader'" :data="logdata.logtable" v-loading="loadinglog" height="100%"
      style="width: 100%">
    ...
    <div class="mb-4">
      <el-pagination background layout="total,prev, pager, next" v-model:current-page="logdata.page"
        v-model:page-size="logdata.size" :total="logdata.total" @size-change="handleSizeChangelog"
