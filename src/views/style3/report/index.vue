  ...
  <div  class="main-boxdiv reportbox" v-loading="loading"> 
          ...
          <td class="el-form-table-tr">
            <span class="mb-4">
              <select v-model="selectid" @change="handleCheckedCitiesChanges">
                ...
                <option
                  v-for="items in home_data.GameList"
        ...
        <tr>
          ...
          <td class="el-form-table-tr">
            <span class="mb-4 report_from">
              <span>
                <el-date-picker
                  v-model="selectData.action_time"
                  type="date"
                  :clearable="false"
                  format="YYYY-MM-DD"
    ...
    <div v-if="showreport">
          <div class="mb-4 mb4_top" style="line-height:20px;">  
            ...
            <el-table border :data="tableData" :height="[tableheight]" style="width: 100%;" tooltip-effect="dark" show-summary :summary-method="getSummaries" :header-cell-style="headerCellStylereptot" :cell-style="reprotlistCellStyle"  :header-cell-class-name="'trheader'" @row-click="handleRowClick" >
              <el-table-column type="index" label="序号" width="50"/>
              <el-table-column :label="getheadertitle()" show-overflow-tooltip>
                <template #default="{ row }">
                  <span v-if="selectData.report_type==2" @click="sendname=getPlayGroup(row),selectData.PlayGroup=row.PlayGroup,selectData.report_type=1,getdata()">
                    {{getPlayGroup(row)}} 
                  </span>
                  <span v-if="selectData.report_type==3" @click="sendname='',selectData.PlayGroup=0,selectData.lottery_id=[parseInt(row.LotteryId)],selectData.report_type=1,getdata()">
                    {{ getLotteryName(row) }}
                  </span>
                  <span v-if="selectData.report_type==4" @click="sendname='',selectData.PlayGroup=0,selectData.report_type=1,selectData.action_time=convertToLocalDate(row.Date),selectData.end_time=convertToLocalDate(row.Date),getdata()">
                    {{convertToLocalDate(row.Date)}} 
                  </span>
                  <span v-if="selectData.report_type==1" @click="checkdown(row)">{{row.Account}}<span v-if="row.Account!='直属会员' && row.Account!='补货'">({{ row.Name }})</span></span>
                </template>
              </el-table-column>  
              ...
              <el-table-column :label="MemberType[nowmember.Type+1]" v-if="nowmember.Type>1 && nowmember.Type<10 && selectData.type==0" show-overflow-tooltip>            
            ...
            <div style="width:100%;height:160px;margin-top:5px;display: none;">
              <div class="report_total">
                  <div>
                    {{MemberType[nowmember.Type]}}: {{parseFloat((tableData.reduce((sum, item) => sum + item.Zyk, 0)).toFixed(2))}}
                  </div>
                  <div v-if="jt">
                    {{MemberType[nowmember.Type-1]}}: {{parseFloat((tableData.reduce((sum, item) => sum + item.Topzcyk, 0)).toFixed(2))}}
                  </div>               
              </div>               
            </div>
        ...
        <el-dialog v-model="showDetail" title="" class="dssetCom-dialog" width="850">
          <div class="titlem bodynav dialog-padding">
           ...
           <component :is="'detail'" :key="dskey" :home_data="home_data" :DetailData="DetailData" :loginmember="loginmember" :MemberType="MemberType"></component>
        </el-dialog>  
